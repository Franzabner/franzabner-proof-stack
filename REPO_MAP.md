...
| Fine-tuning / evaluation lab notes | fine-tuning, evaluation, and lab-note candidates | `local-ai-lab-boundary` and `dgx-ai-workstation-notes` are live as public-safe derivative boundary/workflow scaffolds. Fine-tuning derivative repos still require corpus, private weights, endpoint, log, benchmark, model output, and result review. |
