...
- [ ] `templates/proof-entry.md` requires evidence, status, boundary notes, and review.

## Proof Surface Review

- [ ] Mechanical/Fusion rows do not expose production CAD, geometry, dimensions, or manufacturing packages.
- [ ] Electrical/KiCad/PCB rows do not expose production schematics, routing, BOMs, Gerbers, pin maps, or board source.
- [ ] `public-pcb-review-checklist` is identified as the next concrete engineering proof candidate, not as a published repo or live service offer.
- [ ] Embedded/Pico/RP2350 rows do not expose production firmware, protocols, credentials, or exact integration maps.
- [ ] Agent/harness rows do not expose private YAi/YOSO-YAi prompts, tools, corpora, logs, endpoints, memory, or internals.
- [ ] DGX/NVIDIA/fine-tuning rows do not expose topology, hostnames, ports, credentials, private corpora, private weights, adapters, or training logs.
- [ ] Omniverse/USD/3D rows do not expose production scenes, private CAD, facility layouts, customer assets, or product geometry.
