...
| Risky production-adjacent repos | rack, DGX, agent, Fusion, KiCad production, production RIB, infrastructure lanes | mixed artifacts | Held or derivative-only | May contain private implementation, topology, routing, BOMs, Gerbers, firmware internals, private corpora, weights, prompts, harnesses, or endpoints. `local-ai-lab-boundary` is a public boundary model, not clearance for real infrastructure exposure. | boundary review before edit |
