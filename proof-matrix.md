...
| Risky production-adjacent repos | rack, agent, Fusion, KiCad production, production RIB, infrastructure lanes | mixed artifacts | Held or derivative-only | May contain private implementation, topology, routing, BOMs, Gerbers, firmware internals, private corpora, weights, prompts, harnesses, or endpoints. `local-ai-lab-boundary` and `dgx-ai-workstation-notes` are public boundary/workflow scaffolds, not clearance for real infrastructure exposure. | boundary review before edit |
